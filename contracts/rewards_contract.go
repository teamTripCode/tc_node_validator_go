...
func DistributeRewards(contract *RewardsContract, dpos *consensus.DPoS, blockCreatorAddress string, totalTransactionFees uint64) error {
	...
	// This was a standalone function, not a method of dpos. So it should be consensus.DistributeBlockReward.
	err = consensus.DistributeBlockReward(dpos, blockCreatorAddress)
	if err != nil {
		utils.LogError("RewardsContract.DistributeRewards: Error calling consensus.DistributeBlockReward for %s: %v", blockCreatorAddress, err)
